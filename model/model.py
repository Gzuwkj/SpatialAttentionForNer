...
class CNNNer(nn.Module):
    def __init__(self, model_name, num_ner_tag, cnn_dim=200, biaffine_size=200,
        ...
        self.pretrain_model = AutoModel.from_pretrained(model_name, return_dict=True)
        ...
        config.num_hidden_layers = cnn_depth
    ...
    def forward(self, input_ids, bpe_len, indexes, matrix):
        ...
        max_state = scatter_max(last_hidden_states, index=indexes, dim=1)[0][:, 1:]  # bsz x word_len x hidden_size
        mean_state = scatter(last_hidden_states, index=indexes, dim=1, reduce='mean')[:, 1:]  # bsz x word_len x hidden_size
        state = torch.where()
        ...
        return {'scores': scores, 'attention': attention}
