...
class CNNNer(nn.Module):
    def __init__(self, model_name, num_ner_tag, cnn_dim=200, biaffine_size=200,
        ...
        self.map = nn.Linear(size_embed_dim, cnn_dim)
        ...
        self.down_fc = nn.Sequential(
            nn.Dropout(0.33),
            nn.Linear(cnn_dim, cnn_dim, bias=False),
            nn.GELU(),
            nn.Linear(cnn_dim, num_ner_tag)
        )
        self.logit_drop = logit_drop
        self.norm = nn.LayerNorm(biaffine_input_size, elementwise_affine=False)
    ...
    def forward(self, input_ids, bpe_len, indexes, matrix):
        ...
        # state = scatter(last_hidden_states, index=indexes, dim=1, reduce='mean')[:, 1:]  # bsz x word_len x hidden_size
        ...
        size_embedded = self.dropout(size_embedded).unsqueeze(0).expand(state.size(0), -1, -1, -1)
        size_embedded = self.map(size_embedded)

        scores2 = torch.einsum('bmnh,kh->bkmn', affined_cat, self.W)  # bsz x dim x L x L
        scores = scores2 + scores1 + size_embedded.permute(0, 3, 1, 2)  # bsz x dim x L x L
        ...
        # u_scores1 = self.cnn(u_scores, pad_mask)
        u_scores = self.t2d(
            u_scores,
            mask
        )[0]
